    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for batch_idx, i in enumerate(range(0, len(data), batch_size)):
                ...
                for x in meta_dict: 
                    currImageId=x["id"]
                    currImageName = f'C:/Users/pavan/Downloads/Compressed/flipkart-grid-5-main/flipkart-grid-5-main/downloaded_images/{currImageId}.jpg'
                    if os.path.exists(currImageName):
                        # Open and display the image using PIL
                        img = Image.open(currImageName)
                        img_batch.append(img)
                    else:
                        print(f"The image '{currImageName}' does not exist in the download directory.")
...
def upsert_csv(csv_file, char_enc, pinecone_index, model, bm25):
    try:
        ...
        with open('bm25_john.pkl', 'wb') as f:
            pickle.dump(bm25, f)
...
def main():
    ...
    startTime=time.time()
    csv_file = r"C:\Users\pavan\Downloads\Compressed\flipkart-grid-5-main\flipkart-grid-5-main\backend\dataset\user_history_data\john.csv"
